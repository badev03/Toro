            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    data.forEach(function(obj) {
                        $('#values').append('<option value="' + obj.id +
                            '" >' + obj.value + '</option>');
                    });
        ...
        if (categoryId) {
            $.ajax({
                ...
                success: function(data) {
                    ...
                    // Thêm các option mới từ dữ liệu Ajax
                    data.forEach(function(obj) {
                        $('#value_cate').append('<option value="' + obj.id +
                            '" >' + obj.name + '</option>');
                    });
...
$("#image_path").change(function() {
    $('#image_preview').append('<button class="delete-btn" onclick="deleteAll(this)">Xóa hết</button>');
    ...
    if (this.files && this.files[0]) {
        ...
        for (let i = 0; i < this.files.length; i++) {
            ...
            reader.onload = function(e) {
                ...
                imageItem += '<img style="width: 200px" src="' + e.target.result + '"/>';
                imageItem +=
